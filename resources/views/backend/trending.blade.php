...
@section('content')
                            ...
                            <tbody>
                                @foreach($trendings as $trending)
                                    <tr>
                                        ...
                                        <td>{{ $trending->image }}</td>
                                        <td>{{Illuminate\Support\Str::limit($trending->description, $limit = 100, $end = '...') }}</td>
                                        <td>
                                            @if($trending->is_active )
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu " aria-labelledby="dropdownMenuButton">
                                                    <a href="#" data-id="{{$trending->id}}"
                                                       data-url="{{route('admin.trending.update',$trending->id)}}"
                                                       data-title="{{$trending->title}}"
                                                       data-description="{{$trending->description}}"
                                                       data-active="{{$trending->is_active}}"
                                                       class="dropdown-item js-edit">Edit</a>
                                                    <a href="{{route('admin.trending.destroy',$trending->id)}}"
    ...
    <div class="modal fade" id="addModal" data-backdrop="static" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog">
            <form action="{{route('admin.trending.store')}}" method="post" id="submissionForm" class="submissionForm" enctype="multipart/form-data">
                @csrf
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">New Trending</h4>
    ...
    <div class="modal fade" id="editModal" data-backdrop="static" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog">
            <form action="" method="post" id="submissionFormEdit" class="updateForm" enctype="multipart/form-data">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">Update Trending</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <i aria-hidden="true" class="ki ki-close"></i>
                        </button>
                    </div>
